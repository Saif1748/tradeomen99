...
const StrategyAnalysisTab = ({ trades }: StrategyAnalysisTabProps) => {
  ...
  const insight = generateAIInsights(trades, "strategyAnalysis");
                ...
                return (
                  <tr 
                    ...
                    className={`border-b border-border/50 last:border-0 transition-colors ${
                      isBest ? 'bg-emerald-500/5' : isWorst ? 'bg-rose-500/5' : ''
                    }`}
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        ...
                        {isBest && (
                          <ArrowUp weight="bold" className="w-3.5 h-3.5 text-emerald-400" />
                        )}
                        {isWorst && (
                          <ArrowDown weight="bold" className="w-3.5 h-3.5 text-rose-400" />
                        )}
                      </div>
                    </td>
                    <td className="p-4 text-center text-muted-foreground">{strategy.trades}</td>
                    <td className="p-4 text-center">
                      <span className={strategy.winRate >= 50 ? 'text-emerald-400' : 'text-rose-400'}>
                        {strategy.winRate.toFixed(1)}%
                      </span>
                    </td>
                    <td className="p-4 text-center">
                      <span className={
                        strategy.profitFactor >= 1.5 ? 'text-emerald-400' : 
                        strategy.profitFactor >= 1 ? 'text-foreground' : 'text-rose-400'
                      }>
                        {strategy.profitFactor.toFixed(2)}
                      </span>
                    </td>
                    <td className="p-4 text-center">
                      <span className={strategy.avgPnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}>
                        ${strategy.avgPnl.toFixed(0)}
                      </span>
                    </td>
                    <td className="p-4 text-center">
                      <span className={strategy.totalPnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}>
                        ${strategy.totalPnl.toLocaleString(undefined, { minimumFractionDigits: 0 })}
                      </span>
                    </td>
                    <td className="p-4 text-center">
                      <span className={strategy.maxDrawdown > 15 ? 'text-rose-400' : 'text-muted-foreground'}>
            ...
            <BarChart data={strategyData} layout="vertical">
              ...
              <Bar dataKey="avgPnl" radius={[0, 4, 4, 0]}>
                {strategyData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={entry.avgPnl >= 0 ? "hsl(160 60% 45%)" : "hsl(0 70% 50%)"}
                  />
                ))}
              </Bar>
            </BarChart>
            ...
            <BarChart data={strategyData}>
              ...
              <Bar dataKey="totalPnl" radius={[4, 4, 0, 0]}>
                {strategyData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={entry.totalPnl >= 0 ? "hsl(160 60% 45%)" : "hsl(0 70% 50%)"}
                  />
                ))}
              </Bar>
            </BarChart>
